...
// ৬টি ওরিজিনাল এশিয়ান ট্র্যাডিশনাল প্রতীক পুল তালিকা ভাই ভাই
const symbolsPool = ["FISH", "PRAWN", "CRAB", "ROOSTER", "CALABASH", "COIN"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/fpc-balance', async (req, res) => {
...
// 🛫 ২. ফিশ-প্রন-ক্র্যাব ৩-ডাইস কোর রোলিং রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/fpc-roll', async (req, res) => {
    const { userId, amount, wallet, prediction } = req.body;
    const targetWallet = wallet || "main";
    const reqAmount = parseFloat(amount) || 50;
    
    // প্লেয়ারের সিলেক্ট করা প্রতীক (FISH, PRAWN, CRAB, ROOSTER, CALABASH, COIN)
    const userPrediction = prediction || "FISH"; 
    ...
    try {
        ...
        // 🔒 [ইনসাফিসিয়েন্ট প্রোটেকশন বর্ম]: পকেটে জিরো ব্যালেন্স বা কম টাকা থাকলে বাজি ডিরেক্ট রিফিউজড ভাই ভাই!
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.fpc_target) ? balResponse.data.fpc_target : null;
        ...
        while (isLoopActive && loopSafety < 200) {
            ...
            // ৩টি ডাইসের র্যান্ডম ট্র্যাডিশনাল রেজাল্ট জেনারেটর লক ভাই ভাই
            diceResults = [];
            for (let d = 0; d < 3; d++) {
                diceResults.push(symbolsPool[Math.floor(Math.random() * symbolsPool.length)]);
            }

            // প্লেয়ারের সিলেক্ট করা প্রতীক কয়টি ডাইসে মিলেছে তার গণনা (HIT COUNTER)
            matchCount = diceResults.filter(sym => sym === userPrediction).length;

            if (matchCount > 0) {
                finalStatus = "win";
                // 🚀 [১.৯৫ ক্যাসিনো ওডস ম্যাট্রিক্স]: ইন্টারন্যাশনাল হূ হেই হাউ সুষম হাউস প্রফিট কমিশন বর্ম লক ভাই!
                ...
                else winMultiplier = 5.50;                       // ৩টি ডাইসই মিললে ট্রিপল জ্যাকпот ৫.৫০ গুণ রিটার্ন!
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (adminTriggeredPrize) {
                ...
                if (finalStatus === "win") {
                    // ট্রিপল জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ২% এ লক ভাই ভাই
                    if (matchCount === 3 && Math.random() > 0.02) continue;

                    // ৯৫% আরটিপি সিঙ্ক কন্ট্রোল ম্যাথ লুপ স্বাভাবিক ট্র্যাকে ৩৯% এ ব্যালেন্সড লক ভাই ভাই!
                    if (Math.random() <= 0.39) {
                        isLoopActive = false;
                    }
                } else {
        ...
        console.error("Fish Prawn Crab Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Fish Prawn Crab 3-Dice Engine!"); });

// ফিশ-প্রন-ক্র্যাব গেম নিজস্ব কাস্টম ৫৩০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 5300; 
server.listen(PORT, () => { console.log(`🎡 Royal Fish Prawn Crab Engine Running on port ${PORT}`); });
